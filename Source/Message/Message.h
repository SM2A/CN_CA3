...
class Message
{
    private:
        ...
        uint16_t packet_id, source_port, dest_port, w_size;
        bool is_ack, is_end, is_redundant;
        unsigned char msg[MSG_SIZE] = { 0 }, packet[PACKET_SIZE] = { 0 };

    public:
        Message(unsigned char packet[PACKET_SIZE]);
        Message(uint32_t source_ip, uint32_t dest_ip, uint16_t packet_id, uint16_t source_port, uint16_t dest_port, uint16_t w_size, bool is_ack, bool is_end, bool is_redundant, char msg[MSG_SIZE] = { 0 });
        unsigned char* getMsg() { return msg; }
        unsigned char* getPacket() { return packet; }
        bool isAck() { return is_ack; }
        uint16_t getPacketId() { return packet_id; }
        ...
        uint16_t getWSize() { return w_size; }
